...
public class Lighsaber : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
		...
		//Material mat1 = gameObject.GetComponent<Renderer>().material;

		if (other.gameObject.GetComponent<Rigidbody>().isKinematic == true) makeKinematic = 1;
		...
		newNormal = -transformedNormal * _forceAppliedToCut;
		rigidbody.AddForce(newNormal, ForceMode.Impulse);
		rigidbody.mass = 0.01f;
		rigidbody.useGravity = true;

		slices[0].AddComponent<cubeFragmentScript>();
		slices[1].AddComponent<cubeFragmentScript>();

		//slices[0].GetComponent<Renderer>().material = mat1;
		//slices[1].GetComponent<Renderer>().material = mat1;

		if (makeKinematic == 1)
		{
			float size1 = slices[0].GetComponent<MeshRenderer>().bounds.size.magnitude;
			float size2 = slices[1].GetComponent<MeshRenderer>().bounds.size.magnitude;
			int biggerSlice = 0;
			if (size1 > size2) biggerSlice = 0;
			if (size2 > size1) biggerSlice = 1;
			slices[biggerSlice].GetComponent<Rigidbody>().isKinematic = true;
		}
